...
async function run(args) {
  ...
  if (
    typeof config.app.cache_path === "undefined" ||
  ...
  if (config.app.skip_ps) {
    ...
    ps_data = JSON.parse(
      fs.readFileSync(`${config.app.cache_path}/ps_data.json`, {
        encoding: "utf8",
      })
    );
    console.log(
      log.info + "`skip_ps` Set! Skiping Powerschool Data Retreival..."
    );
  } else {
  ...
  if (config.app.skip_ad) {
    ad_data = JSON.parse(
      fs.readFileSync(`${config.app.cache_path}/ad_data.json`),
      {
        encoding: "utf8",
      }
    );
    console.log(
      log.info + "`skip_ad` Set! Skipping Active Directory Data Retreival..."
    );
  } else {
  ...
  if (config.app.initial) {
    ...
    if (config.app.verbose) {
      ...
      fs.writeFileSync(
        `${config.app.cache_path}/tmp_migrate_data.json`,
        JSON.stringify(ad_with_dcid, null, 2),
        { encoding: "utf8" }
      );
    }
  } else {
...
async function setupPowerSchool(config) {
  ...
  fs.writeFileSync(
    `${config.app.cache_path}/ps_data.json`,
